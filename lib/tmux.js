...
exports.createTmuxWindow = async (session, window, wtd, focus = false, cmd = '') => {
  ...
  if (!await windowExists(session, window)) {
    ...
    // console.log(`${direnvPrefix()}tmux new-window ${focusFlag}-t ${session} -n ${window} -c ${wtd}${cmdSuffix}`)
    await command(`${direnvPrefix()}tmux new-window ${focusFlag}-t ${session} -n ${window} -c ${wtd}${cmdSuffix}`, {shell: true});
  } else {
...
exports.switchSession = async (session, window, wtd, focus = false) => {
