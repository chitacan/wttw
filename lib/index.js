...
const {getConfig, setConfig, requireConfig, loadTemplate, listTemplates, loadProjectConfig, getProjectConfig, setProjectConfig, resolveHooksConfigDir} = require('./config');
const {
...
cmd.command('new <branches...>')
  ...
  .option('-H, --no-hook', 'skip running hooks')
  .option('-f, --focus', 'focus created tmux window (Default: config focus)')
  .option('-c, --cmd <command>', 'command to run in each pane after creation. Supports <%= branch %>, <%= window %> variables. (Default: config cmd)')
  .option('-S, --no-stash', 'skip moving changes to new worktree')
  .action(async (branches, {pane: paneOpt, session: sessionOpt, baseRef: baseRefOpt, detach, workspace, hook, tmux: tmuxOpt, cmd: cmdOpt, focus: focusOpt, stash: stashOpt, parent: {dryRun}}) => {
    try {
      const cwd = await rootDir();
      const config = loadProjectConfig(cwd);
      let pane = paneOpt ?? config.paneCount;
      let session = sessionOpt ?? config.session;
      let tmux = Boolean(tmuxOpt ?? config.tmux);
      let focus = Boolean(focusOpt ?? config.focus);
      ...
      const dirty = stashOpt && await isDirty();
      if (dirty && !dryRun) await stash();
      ...
      for (const branch of branches) {
        ...
        const renderedCMD = renderCMD(cmdOpt ?? config.cmd, {window, branch});
        ...
        if (dryRun) {
          ...
          debug('cmd               : %s', renderedCMD);
          debug('focus             : %s', focus);
          debug('code-workspace dir: %s', workspacesDir)
          debug('hooks             : %s', JSON.stringify(config.hooks || []))
        } else {
          await createWorktree(wtd, branch, baseRef, detach)
          if (dirty) await stashApply(wtd);
          copyDefaultFiles(defaultFiles, wtd)
          if (workspace) copyWorkspaceFile(workspacesDir, mainWorktree, wtd)
          if (hook) await runHook(config.hooks, {window, wtd, branch, configDir: resolveHooksConfigDir(cwd)})

          note(`worktree ${branch} (on ${baseRef}) created`);

          if (tmux) {
            await switchSession(session, window, wtd, focus);
            await createTmuxWindow(session, window, wtd, focus, renderedCMD)
            await splitTmuxWindow(`${session}:${window}`, wtd, +pane)
          }
        }
      }

      if (dirty && !dryRun) await stashDrop();
    } catch (err) {
      error(err);
      process.exit(1);
    }
...
cmd.command('hook [branch]')
  .alias('h')
  .description('run hooks on an existing worktree')
  .option('-i, --index <index>', 'run only the hook at this index')
  .action(async (branch, {index, parent: {dryRun}}) => {
    try {
      ...
      if (!existsSync(wtd)) {
        throw new Error(`worktree "${branch}" does not exist at ${wtd}`);
      }
      ...
      if (index !== undefined) {
        ...
        if (i < 0 || i >= hooks.length) {
          throw new Error(`hook index ${i} out of range (0-${hooks.length - 1})`);
        }
        hooks = [hooks[i]];
      }
      ...
      if (dryRun) {
        ...
        debug('window : %s', window);
        debug('wtd    : %s', wtd);
        debug('hooks  : %s', JSON.stringify(hooks));
      } else {
        const errors = await runHook(hooks, {window, wtd, branch, configDir: resolveHooksConfigDir(cwd)});
        if (errors.length > 0) {
...
cmd.command('default-files')
  .alias('df')
  .description('copy default_files to current directory. Supports <%= branch %> variable.')
  .action(async () => {
    try {
      await checks({requireTmux: false});
...
cmd.command('tmux <branches...>')
  ...
  .option('-f, --focus', 'focus created tmux window (Default: config focus)')
  .option('-c, --cmd <command>', 'command to run in each pane after creation. Supports <%= branch %>, <%= window %> variables. (Default: config cmd)')
  .action(async (branches, {pane: paneOpt, session, focus: focusOpt, cmd: cmdOpt, parent: {dryRun}}) => {
    try {
...
cmd.command('config [key] [value]')
  ...
  .on('--help', () => {
    ...
    console.log('    focus              focus created tmux window (boolean)');
    ...
    console.log('    hooks              hooks to run after worktree creation (array, edit JSON directly)');
    console.log('');
