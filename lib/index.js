...
cmd.command('new <branch>')
  ...
  .action(async (branch, {pane, session, baseRef, detach, workspace, tmux, cmd, parent: {dryRun}}) => {
    try {
      ...
      if (dryRun) {
        ...
        if (tmux) {
          ...
          await createTmuxWindow(session, window, wtd, cmd)
...
cmd.command('tmux <branch>')
  .alias('t')
  .description('open worktree in tmux window')
  .option('-p, --pane <count>', 'tmux pane count', DEFAULT_PANE_COUNT)
  .option('-s, --session [session]', 'session name for tmux window (Default: current session)')
  .option('-c, --cmd <command>', 'command to run in each pane after creation')
  .action(async (branch, {pane, session, cmd, parent: {dryRun}}) => {
    try {
