...
const {getConfig, setConfig, requireConfig, loadTemplate} = require('./config');
const {
...
cmd.command('new <branches...>')
  ...
  .option('-p, --pane <count>', 'tmux pane count', getConfig('defaultPaneCount'))
...
cmd.command('open <branch>')
  ...
  .option('-h, --host [host]', 'machine to open worktree', getConfig('github.host'))
  .option('-p, --profile [profile]', 'vscode profile', getConfig('vscode.profile'))
...
cmd.command('template <template-name>')
  ...
  .on('--help', () => {
    ...
    console.log('    windows[].pane   pane count (optional, default: config defaultPaneCount)');
  ...
  .action(async (templateName, {parent: {dryRun}}) => {
    try {
      ...
      const defaultPaneCount = getConfig('defaultPaneCount');
      ...
      for (const w of windows) {
        if (dryRun) {
          debug('=== %s ===', w.name);
          debug('path   : %s', w.path);
          debug('pane   : %s', w.pane || defaultPaneCount);
          debug('cmd    : %s', w.cmd || '');
          debug('session: %s', session);
        } else {
          const wtd = resolvePath(w.path);
          await createTmuxWindow(session, w.name, wtd, false, w.cmd || '')
          await splitTmuxWindow(`${session}:${w.name}`, wtd, w.pane || defaultPaneCount)
        }
      }
  ...
  .action((key, value) => {
    try {
      if (!key) {
        ...
        setConfig(key, isNaN(parsed) ? value : parsed);
        note(`${key} = ${value}`);
      }
    } catch (err) {
      error(err);
