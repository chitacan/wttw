...
describe('createTmuxWindow', () => {
  ...
  test('window not exists', async () => {
    ...
    expect(command)
      .toHaveBeenNthCalledWith(1, `tmux list-windows -t ${session} -F #W`);
    expect(command)
      .toHaveBeenNthCalledWith(2, `tmux new-window -d -t ${session} -n ${window} -c ${wtd}`, {shell: true});
  ...
  test('window not exists with focus', async () => {
    ...
    expect(command)
      .toHaveBeenNthCalledWith(2, `tmux new-window -t ${session} -n ${window} -c ${wtd}`, {shell: true});
  });
  ...
  test('window not exists with cmd', async () => {
    ...
    expect(command)
      .toHaveBeenNthCalledWith(2, `tmux new-window -d -t ${session} -n ${window} -c ${wtd} vim`, {shell: true});
  });
