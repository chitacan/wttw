...
describe("createTmuxWindow", () => {
  test("ok", async () => {
    ...
    expect(cmd1).toBe(`tmux new-window -n ${window} -c ${wtd}`);
    expect(cmd2).toBe(`tmux select-window -t ${window}`);
  });
});
...
describe("splitTmuxWindow", () => {
  test("4 pane", async () => {
    const wtd = 'worktree';
    await utils.splitTmuxWindow(wtd, 4);
  ...
  test("6 pane", async () => {
    const wtd = 'worktree';
    await utils.splitTmuxWindow(wtd, 6);
    const [cmds] = command.mock.calls;
    expect(cmds.length).toBe(2);
  });
});
