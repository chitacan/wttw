...
exports.isDirty = async () => {
  return await command('git status --porcelain')
    .then(({stdout}) => stdout.trim().length > 0)
}

exports.stash = async () => {
  await command('git stash --include-untracked')
}

exports.stashApply = async (cwd) => {
  await command('git stash apply', {cwd}).catch(() => {})
}

exports.stashDrop = async () => {
  await command('git stash drop')
}
