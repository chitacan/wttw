...
const windowExists = async (session, window) => {
  try {
    const windows = await command(`tmux list-windows -t ${session} -F #W`)
      .then(({stdout}) => stdout);
...
const sessionExists = async (name) => {
  try {
    const sessions = await command('tmux list-windows -F #S')
      .then(({stdout}) => stdout);
...
exports.createTmuxWindow = async (session, window, wtd) => {
  ...
  if (!await windowExists(session, window)) {
    await command(`tmux new-window -t ${session} -n ${window} -c ${wtd}`);
  }
...
exports.splitTmuxWindow = async (target, wtd, pane) => {
  if (pane === 4) {
    await command(`tmux splitw -v -p 50 -t ${target}.0 -c ${wtd} && \
                tmux splitw -h -p 50 -t ${target}.0 -c ${wtd} && \
                tmux splitw -h -p 50 -t ${target}.2 -c ${wtd} && \
                tmux selectp -t ${target}.0`, {shell: true})
  } else if (pane === 6) {
    await command(`tmux splitw -v -p 50 -t ${target}.0 -c ${wtd} && \
                tmux splitw -h -p 33 -t ${target}.0 -c ${wtd} && \
                tmux splitw -h -p 50 -t ${target}.0 -c ${wtd} && \
                tmux splitw -h -p 33 -t ${target}.3 -c ${wtd} && \
                tmux splitw -h -p 50 -t ${target}.3 -c ${wtd} && \
                tmux selectp -t ${target}.0`, {shell: true})
  }
}
...
const createSession = exports.createSession = async (session, window, wtd) => {
  return await await command(`tmux new-session -d -c ${wtd} -s ${session} -n ${window}`);
}
