...
exports.copyDefaultFiles = (defaultFiles, wtd) => {
  ...
  const paths = klawSync(defaultFiles, {nodir: true, traverseAll: true})
  .map(item => item.path)
...
exports.runHook = async (hooks, {window, wtd, branch, configDir}) => {
  ...
  for (const hook of hooks) {
    try {
      if (hook.shell) {
        await command(hook.shell, {
          cwd: wtd,
          env: {...process.env, WTTW_WINDOW: window, WTTW_WTD: wtd, WTTW_BRANCH: branch},
        });
      } else if (hook.js) {
        const jsPath = isAbsolute(hook.js) ? hook.js : resolve(configDir, hook.js);
        await require(jsPath)({window, wtd, branch});
      }
    } catch (err) {
      debug('hook failed: %s %s', JSON.stringify(hook), err.message);
      errors.push(err);
    }
  }
  return errors;
...
exports.renderCMD = (cmd = "", {window, branch}) => render(cmd, {window, branch});
