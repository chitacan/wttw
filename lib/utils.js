...
exports.createTmuxWindow = async (session, window, wtd, cmd = '') => {
  const targetSession = session ? session + ':' : '';
  if (!await windowExists(session, window)) {
    await command(`${direnvPrefix()}tmux new-window -t ${session} -n ${window} -c ${wtd} ${cmd}`);
  }
...
exports.buildCodeURL = (wtd, window, profile) => {
  ...
  url.pathname = join('tunnel', 'cmms', existsSync(wsd) ? wsd : wtd);
...
exports.requestOpenCode = async (url, host) => {
  const slug = {owner: 'chitacan', repo: 'wttw'};
